...
function ExG.RosterFrame:RenderList()
    ...
    for i = 1, ttlMembers do
        ...
        local ep, gp = ExG:FromString(offNote);

        tinsert(roster, { name = Ambiguate(name, 'all'), rank = rank, level = level, class = class, classLoc = classLoc, offNote = offNote, isOnline = isOnline, ep = ep, gp = gp });
    end
