...
L['Loot pattern'] = '(.+)%sreceive%sloot:%s(|cff.+|h|r).*';
L['Loot tracker message'] = function(link) return format('You have to give %s to masterlooter!', link); end;
L['Track loot'] = true;
L['Time is out'] = function(left) if left > 0 then return format('|cff33ff99%d sec|r', left); else return '|cff33ff99Time is out|r'; end end;
