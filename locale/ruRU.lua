...
L['Auto loot items'] = 'Автосбор предметов';
L['Track loot'] = 'Отслеживать лутом';
L['Loot pattern'] = '(.+)%sполучает%sдобычу:%s(|cff.+|h|r).*';
L['Loot tracker message'] = function(link) return format('Нужно передать %s мастерлутеру!', link); end;
L['Time is out'] = function(left) if left > 0 then return format('|cff33ff99%d сек|r', left); else return '|cff33ff99Время вышло|r'; end end;
...
L['Officer'] = 'Офицер';
L['officer'] = 'Офицер';
L['Group Title'] = function(number) return format('Группа %d', number or 0); end;
...
L['Join Reserve'] = 'В резерв';
L['Add Reserve Player'] = 'Добавить игрока';
